...
const AssignedReviews: React.FC = () => {
  ...
  // Fetch require_quiz / quiz_taken per assignment
  useEffect(() => {
    axiosClient
  ...
  useEffect(() => {
    ...
    const buildRows = (
      ...
      maps.map((map) => {
        ...
        return {
          ...
          // per-team quiz state from the API
    ...
    // 1) Check localStorage (written by AssignReviewer) if scoped to one assignment
    if (resolvedAssignmentId) {
      try {
    ...
    // 2) Fetch all reviews from backend
    let cancelled = false;
    axiosClient
            ...
            <table className="table table-striped" style={{ fontSize: 15, lineHeight: "1.428em" }}>
              ...
              <tbody>
                {assignedReviews.map((review) => {
                  ...
                  // Use per-row quiz state so each team's quiz is tracked separately
                  ...
                  const handleTakeQuiz = async () => {
                    ...
                    try {
                      const res = await axiosClient.post('/quiz_response_maps', {
                        ...
                        // Required so backend creates the quiz map for the correct team
